...
- [Architecture](./architecture.md): request flow, storage layout, workers, and status calculation.
...
- [Development](./development.md): test and validation commands, admin TUI workflow, and code hotspots.
- [Troubleshooting](./troubleshooting.md): common startup and runtime failures.

## Project summary

`aretheyup` is a Go application that serves public status pages and private same-site API endpoints backed by PostgreSQL and Redis. The binary can run as:
