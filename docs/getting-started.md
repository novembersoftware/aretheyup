...
The worker mode refreshes baselines, refreshes status snapshots, reconciles incidents, logs table-size estimates, and cleans up expired raw probe results.
...
Runs recurring database jobs for baseline refresh, status snapshot refresh, incident reconciliation, table-size logging, and raw probe history cleanup. Implemented in `main.go`, `workers/baseline.go`, `workers/statuses.go`, `workers/incidents.go`, `workers/cleanup.go`, and `workers/table_stats.go`.
...
- `go run main.go worker` logs baseline, status, incident, cleanup, and table-stats worker startup.
