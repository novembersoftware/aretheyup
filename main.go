...
func workerMode(store *storage.Storage) {
	...
	workers.StartIncidentTracker(store, config.StatusSnapshotIncidentReadsEnabled())
	workers.StartProbeResultCleaner(store, config.RawProbeRetentionCleanupEnabled())
