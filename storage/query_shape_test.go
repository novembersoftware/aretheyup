...
func TestListAndIncidentPathsAvoidRawProbeQueries(t *testing.T) {
	storageSource := mustReadSource(t, "storage.go")
	incidentSource := mustReadSource(t, "../workers/incidents.go")

	guarded := map[string]string{
		"ListServices":       between(t, storageSource, "func (s *Storage) ListServices", "// SearchServices"),
		"SearchServices":     between(t, storageSource, "func (s *Storage) SearchServices", "// GetServiceBySlug"),
		"reconcileIncidents": between(t, incidentSource, "func reconcileIncidents", "func statusFromSnapshot"),
	}
	...
	for _, name := range []string{"ListServices", "SearchServices"} {
