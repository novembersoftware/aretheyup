...
func TestRefreshServiceStatusDoesNotOverwriteNewerSnapshot(t *testing.T) {
	store, db := newStatusIntegrationStore(t)
	ctx := context.Background()

	service := createStatusTestService(t, db)
	...
	if _, err := store.RefreshServiceStatus(ctx, service.ID, olderComputedAt); err != nil {
		t.Fatalf("RefreshServiceStatus older snapshot error = %v", err)
	}
